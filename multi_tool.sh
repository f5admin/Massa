...
# Options
. <(wget -qO- https://raw.githubusercontent.com/f5nodes/logo/colors.sh) --
option_value(){ echo "$1" | sed -e 's%^--[^=]*=%%g; s%^-[^=]*=%%g'; }
while test $# -gt 0; do
	case "$1" in
	-h|--help)
		. <(wget -qO- https://raw.githubusercontent.com/f5nodes/logo/logo.sh)
		echo
...
open_ports() {
	...
	. <(wget -qO- https://raw.githubusercontent.com/f5nodes/logo/ports_opening.sh) 31244 31245
...
WantedBy=multi-user.target" > /etc/systemd/system/massad.service
		...
		. <(wget -qO- https://raw.githubusercontent.com/f5nodes/logo/insert_variable.sh) -n massa_wallet_address -v "$wallet_address"
...
WantedBy=multi-user.target" > /etc/systemd/system/massad.service
			...
			. <(wget -qO- https://raw.githubusercontent.com/f5nodes/logo/insert_variable.sh) -n massa_wallet_address -v "$wallet_address"
			. <(wget -qO- https://raw.githubusercontent.com/f5nodes/logo/insert_variables.sh)
			if [ ! -d $HOME/massa_backup ]; then
...
"
		...
		. <(wget -qO- https://raw.githubusercontent.com/f5nodes/logo/insert_variable.sh) -n massa_wallet_address -v "$wallet_address"
		. <(wget -qO- https://raw.githubusercontent.com/f5nodes/logo/insert_variable.sh) -n massa_log -v "sudo journalctl -f -n 100 -u massad" -a
	fi
	printf_n "${C_LGn}Done!${RES}"
	cd
	. <(wget -qO- https://raw.githubusercontent.com/f5nodes/logo/logo.sh)

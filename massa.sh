...
# Options
. <(wget -qO- https://raw.githubusercontent.com/f5eleven/logo/colors.sh) --
option_value(){ echo "$1" | sed -e 's%^--[^=]*=%%g; s%^-[^=]*=%%g'; }
while test $# -gt 0; do
	case "$1" in
	-h|--help)
		. <(wget -qO- https://raw.githubusercontent.com/f5eleven/logo/logo.sh)
		echo
		...
		echo -e "https://github.com/f5eleven/massa/massa.sh - script URL"
...
open_ports() {
	...
	. <(wget -qO- https://raw.githubusercontent.com/f5eleven/logo/ports_opening.sh) 31244 31245
...
WantedBy=multi-user.target" > /etc/systemd/system/massad.service
		...
		. <(wget -qO- https://raw.githubusercontent.com/f5eleven/logo/insert_variable.sh) -n massa_wallet_address -v "$wallet_address"
		. <(wget -qO- https://raw.githubusercontent.com/f5eleve/logo/insert_variables.sh)
		cd
		. <(wget -qO- https://raw.githubusercontent.com/f5eleven/logo/logo.sh)
		printf_n "
...
WantedBy=multi-user.target" > /etc/systemd/system/massad.service
			...
			. <(wget -qO- https://raw.githubusercontent.com/f5eleven/logo/insert_variable.sh) -n massa_wallet_address -v "$wallet_address"
			. <(wget -qO- https://raw.githubusercontent.com/f5eleven/logo/insert_variables.sh)
			if [ ! -d $HOME/massa_backup ]; then
...
install_source() {
	if [ -d $HOME/massa/ ]; then
		...
		sudo apt install unzip jq curl pkg-config git build-essential libssl-dev -y
		printf_n "${C_LGn}Node installation...${RES}"
		. <(wget -qO- https://raw.githubusercontent.com/f5ele/logo/rust.sh) -n
		if [ ! -d $HOME/massa/ ]; then
...
"
		...
		. <(wget -qO- https://raw.githubusercontent.com/f5eleven/logo/insert_variable.sh) -n massa_wallet_address -v "$wallet_address"
		. <(wget -qO- https://raw.githubusercontent.com/f5eleven/logo/insert_variable.sh) -n massa_log -v "sudo journalctl -f -n 100 -u massad" -a
	fi
	printf_n "${C_LGn}Done!${RES}"
	cd
	. <(wget -qO- https://raw.githubusercontent.com/f5eleven/logo/logo.sh)
